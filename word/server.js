...
function ToShowTips(){
    ...
    close.id = "popWindowClose";
    let content = document.createElement("div");
    content.innerHTML = "<div id='wordContent'>复习完毕</div>"+
                        "<div id='translation'>请保存测试结果</div>";
...
var faultIndex = [];
const faultTempt = new Set();//存放记忆错词汇
//顺序点击
...
function SaveRememberPoint(num){
    ...
    SetCookie("上回的单词次序",num);
    SetCookie("上回没记住的词汇下标",JSON.stringify(faultIndex));
    ToShowTips1(1);
}
//随机记单词保存选择错误单词下标
function SaveRememberPointByRandom(num){
    ...
    SetCookie("随机没记住的单词下标",JSON.stringify(faultIndex));
    ToShowTips1(1);
}
//复习记单词保存选择错误单词下标
function SaveRememberPointByReview(num){
    faultIndex = SetToArray(faultTempt);
    // SetCookie("上次随机到第几个单词",num);
    SetCookie("复习没记住的单词下标",JSON.stringify(faultIndex));
    ToShowTips1(1);
}
